...
RUN if [ "$PRELOAD_HF" = "1" ]; then \
      python -c "import os; from transformers import pipeline; m=os.getenv('HF_MODEL_ID','microsoft/DialoGPT-medium'); p=pipeline('text-generation', model=m); p('hello', max_length=5); print('Preloaded Hugging Face model:', m)"; \
    fi
