...
class HuggingFaceProvider(LLMProvider):
    ...
    async def generate_response(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            response_time_ms = int((time.time() - start_time) * 1000)
            content = response[0]["generated_text"] if response else "I'm not sure how to respond."
...
class LLMService:
    ...
    async def initialize(self):
        ...
        # Set the first healthy provider as current
    ...
    async def _select_healthy_provider(self):
        ...
        for provider in self.providers:
            try:
                if await provider.is_healthy():
                    self.current_provider = provider
                    logger.info(f"Selected provider: {provider.get_provider_name()}")
                    return
            except Exception:
                continue
